...
- Get basic button example: component="button", section="basic"`,
  ...
  inputSchema: {
    ...
    section: z
      .string()
      .describe('The section id (e.g., "basic", "severity", "icon")'),
  },
  ...
  handler: async ({ component: name, section }) => {
    try {
      ...
      // Find the example
      const example = component.sections.find(
        (s) => s.id.toLowerCase() === section.toLowerCase(),
      );
      if (!example) {
        return {
          content: [
            {
              type: 'text',
              text: `Example "${section}" not found for component "${component}". Available examples: ${component.sections
                .filter((it) => it.hasExample)
                .map((s) => s.id)
                .join(', ')}`,
            },
          ],
