...
```
GET /api/products/options?category=true&inventoryStatus=true
```
...
The main products endpoint includes CORS headers to support cross-origin requests:
...
- **Products endpoint** (`/api/products`): Simulates 2-second delay for network latency
- **Options endpoint** (`/api/products/options`): Simulates 500ms delay
