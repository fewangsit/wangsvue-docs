...
export default async function generateUnifiedData() {
  ...
  try {
    ...
    for (const file of componentFiles) {
      ...
      componentData.sections = await Promise.all(
        sections.map(async (section) => {
          ...
          const demoMatch = section.originalContent?.match(
            /::component-demo\{src="([^"]+)"\}/g,
          );
          if (demoMatch) {
...
function parseMarkdownSections(content) {
  ...
  while ((match = sectionRegex.exec(content)) !== null) {
    ...
    const demoMatch = sectionContent.match(
      /::component-demo\{src="([^"]+)"\}/g,
    );
...
async function main() {
  try {
    ...
    console.log(
      `✅ Generated unified data for ${components.length} components`,
    );
    ...
    const totalExamples = components.reduce((count, component) => {
      return (
        count +
        component.sections.filter((section) => section.hasExample).length
      );
    }, 0);
    console.log(
      `📄 Included ${totalExamples} component examples with source code`,
    );
    ...
    components.forEach((component) => {
      ...
      component.sections.forEach((section) => {
        ...
        console.log(
          `   - ${section.label}: ${section.description.substring(0, 50)}...${hasCode}`,
        );
      });
    });
  } catch (error) {
